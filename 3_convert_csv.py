...
import os
from helper import get_csv, write_csv, linebreak_to_txt, linebreak_to_html

def get_args():
	import argparse
	parser = argparse.ArgumentParser(description='Cleans and converts CSV (removes line-breaks, ...)')
	parser.add_argument('--in_folder', type=str, default='../anki')
	parser.add_argument('--out_folder', type=str, default='../anki/converted')
...
def main():
	...
	files = glob(os.path.join(args.in_folder, '*.csv'))
	for file in files:
		name = file.rsplit('/')[-1].rsplit('.', 1)[0]
		print(name)
		new_filename = os.path.join(args.out_folder, name + '.csv')
		lines = get_csv(file)
